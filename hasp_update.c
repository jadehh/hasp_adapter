...
#include "include/hasp_vcode.h"   /* this header file holds the Sentinel DEMOMA vendor code */
...
static unsigned char *read_file(FILE *fp)
{
    ...
    unsigned int   buffer_length, file_size;
    ...
    buffer_length = (unsigned int)fread(buffer, sizeof(char), file_size, fp);
    if (buffer_length <= 0)
    {
